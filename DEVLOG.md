...
## Итоги дня · 18 августа 2026

### Сделано
- **4.10 ✅** фильтры: плюс/минус отдельно для заголовка и текста, русские склонения (`сайт` → сайты/сайтов). Режет хаб, VPS не рестартили
- Слепки хаба: `npm run snapshot` + ночной cron 03:10 (`_backups/leads/` — БД + `.env` + git bundle). На GitHub пушнули теги `checkpoint/filters-4.10-stem-20260818`, `rollback-2026-08-14`
- **Операторский счёт:** VPS +40 ₽/день (сумма к концу периода); разовое подключение 5 000 ₽; каждый месяц API ИИ $20 = 1 700 ₽; со 2-го месяца поддержка 2 000 ₽
- Флажок **оплачен / не оплачен** в админке, тот же статус у партнёра (меню «Счёт» + полоска на обзоре)
- Календарь оплаты (текущий период + следующие)
- Цены правит админ: шаблон для новых + свои суммы у каждого партнёра (`Оператор → Счета`)
- Пауза / стоп / продлить / без остановки — без рестарта VPS-агента

### Файлы
- `src/lib/billing/operator-pricing.ts`, `quota.ts`, `payment-calendar.ts`, `view.ts`
- `src/components/billing/*`, `src/app/dashboard/admin/billing/page.tsx`, `src/app/dashboard/billing/page.tsx`
- `src/lib/leads/ru-stem.ts`, `partner-filters.ts` (уже в git с утра)
- `prisma/migrations/20260818120000_operator_billing`, `20260818130000_period_paid_calendar`, `20260818140000_billing_price_lines`
- `scripts/backup-snapshot.sh` (уже в git)
- `docs/PLAN_2026-08-10.md`, `DEVLOG.md`

### Production (закрытие дня)
- `npm run build` ✅ · `localhost:3005` → **200**
- PM2: `leads-konversus` / `leads-health` **online**; `leads-profi` нет
- Пилот `RysyevIV`: HB свежий, CB **CLOSED**, lastError нет
- Заявки сегодня (МСК): **14** в БД; всего **2212**
- Счёт пилота: период 1, подключение в базе уже «оплачено» (5 000 не всплывают), период **не оплачен** пока не отметите флажок
- `profiOnHub: false` соблюдён; `leads-agent-v2` **не** рестартили

### Осталось / завтра
- По желанию: 4.11.3 утренняя сводка; TG-напоминание «счёт не оплачен»; копия слепка на другой диск/VPS
- **Не** начинать этап 6 AI и 4.9 бот без команды
- Не рестартить пилотный агент без явной просьбы


## 2026-08-20

### Файл: виджет-калькулятор для bani53.ru (предпросмотр)

**Проблема:** на bani53.ru (конструктор Крафтум) нужен современный калькулятор стоимости бань по данным каталога. На сайте была только форма-опрос «Расчет стоимости» без подсчёта цены.

**Файлы:** `public/bani53-calculator.html` — самодостаточный HTML/CSS/JS-виджет для вставки в Крафтум блоком «HTML-код». Каталожные цены (Стандарт 4–7 м, С террасой 5–7 м, Скандинавия 4,5–8 м), линейная интерполяция нестандартных длин, опция «Санузел» +10 000 ₽, доставка: бесплатно до 500 км от Пестово, далее 160 ₽/км (геокодинг Nominatim), кнопка WhatsApp с предзаполненным расчётом.

**Результат:** проверен в Playwright на Nordic: рендер, переключение серий, слайдер длины, геокодинг «Вологда» (≈245 км → бесплатно), ошибок консоли нет. Логика покрыта node-тестом (24 проверки). Файл также сохранён локально у заказчика для вставки в Крафтум.

## 2026-08-20 (2)

### Файл: виджет-калькулятор bani53.ru — мобильная версия и кнопка MAX

**Проблема:** на мобильных часть калькулятора вылезала за края экрана; кнопка WhatsApp не нужна — заказчик переходит на мессенджер MAX.

**Файлы:** `public/bani53-calculator.html` (v1.1.0) — мобильные media-запросы перенесены в конец <style> (каскад), сетка minmax(0,1fr), серии в одну строку, компактная липкая итоговая панель (sticky bottom), кнопка «Написать в MAX» на https://max.ru/u/f9LHodD0cOJGd17TGHQs-RQFEHl-1LirxFAUAjz9LuuaUG8WxyuQM_y9Qfc, кнопка звонка — иконкой.

**Результат:** Playwright 360×740: высота виджета 742px (в один экран), горизонтального переполнения нет (scrollWidth = viewport), ошибок консоли нет. Логика расчёта — 24/24 node-тестов.

## 2026-08-20 (3)

### Файлы: PDF-каталог для bani53.ru

**Проблема:** заказчику нужен красивый каталог бань в PDF.

**Файлы:** `public/bani53-catalog.html` (вёрстка) и `public/bani53-catalog.pdf` (9 страниц A4, рендер Playwright/Chromium). Обложка, о компании, серии Стандарт/С террасой/Санузел/Скандинавия (14 моделей с фото и ценами с сайта), базовая комплектация, доставка и условия, контакты.

**Результат:** PDF 3,28 МБ, 9 страниц, все изображения загружены с selcdn (waitForFunction по naturalWidth>0). Локальная копия — у заказчика.

---

## Итоги дня · 20 августа 2026

### Проект: bani53.ru (Крафтум) — калькулятор и PDF-каталог

**Сделано:**
- Исследован сайт bani53.ru (конструктор Крафтум, одностраничник): каталог 15 моделей, цены, комплектация, доставка (бесплатно до 500 км, далее 160 ₽/км), телефон, MAX/VK.
- Создан виджет-калькулятор стоимости бани `public/bani53-calculator.html` v1.1.0: серии Стандарт/С террасой/Скандинавия, линейная интерполяция, санузел +10 000 ₽, доставка по расстоянию (Nominatim), кнопка «Написать в MAX», тёплый стиль в цветах сайта.
- Мобильная вёрстка: без горизонтального переполнения, виджет 742px — в один экран 360×740, липкая итоговая панель.
- Создан PDF-каталог `public/bani53-catalog.pdf` (9 страниц A4, 3,28 МБ, фото с selcdn) + исходник `public/bani53-catalog.html`.

**Проверки:**
- Логика калькулятора: node-тест 24/24.
- Виртуальный браузер Playwright: desktop + mobile 360×740 — переполнения нет, ошибок консоли нет, геокодинг «Вологда» работает.
- PDF: 9 страниц, валидный %PDF-1.4, все изображения загружены.
