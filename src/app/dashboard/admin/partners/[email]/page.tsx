// Страница партнёра — всё на одном экране для админа
import { db } from "@/lib/db";
import { auth } from "@/lib/auth/auth";
import { notFound } from "next/navigation";
import { Shield, Activity, Inbox, Plug, Settings, Clock, AlertTriangle } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 24, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontSize: "var(--text-2xl)", fontWeight: 700, marginBottom: 2 }}>
            {partner.firstName || email.split("@")[0]}
          </h1>
          <p style={{ color: "var(--ink-muted)", fontSize: "var(--text-sm)" }}>{email}</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0, border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", overflow: "hidden" }}>
        ...
        <div style={{ padding: "20px 24px", background: "var(--bg-surface)", borderRight: "1px solid var(--border)" }}>
          <h2 style={{ fontSize: "var(--text-sm)", fontWeight: 650, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {/* ═══ Правая колонка: Источник + Telegram ═══ */}
        <div style={{ padding: "20px 24px", background: "var(--bg-surface)" }}>
          <h2 style={{ fontSize: "var(--text-sm)", fontWeight: 650, marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
            <Plug size={16} /> Источник
          </h2>
          ...
          {source ? (
            <>
              ...
              <Row label="Статус" value={
                (source.status || (source.enabled ? "active" : "pending")) === "active" ? "🟢 Работает" :
                source.status === "error" ? "🔴 Ошибка" :
                source.status === "pending" || !source.enabled ? "🟡 Ожидает проверки" : (source.status || "—")
              } />
              {source.lastError && (
                <Row label="Ошибка" value={source.lastError} mono />
              )}
              <Row label="Логин Profi" value={(source.config as any)?.login || "—"} />
              ...
              {/* Кнопка проверки */}
              {source.platform === "profi" && (
                <div style={{ marginTop: 12 }}>
                  <ProfiTestButton
                    sourceId={source.id}
                    currentLogin={(source.config as any)?.login || ""}
                    currentPassword={(source.config as any)?.password || ""}
                  />
                </div>
              )}
            </>
          ...
          <h2 style={{ fontSize: "var(--text-sm)", fontWeight: 650, marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
            📱 Telegram
          </h2>
          ...
          {sub?.expiresAt && (
            <Row label="До" value={`${new Date(sub.expiresAt).toLocaleDateString("ru")} (${daysLeft} дн.)`} />
          )}
        </div>
      </div>
      ...
      <div style={{ marginTop: 24, border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", overflow: "hidden", background: "var(--bg-surface)" }}>
        <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <h2 style={{ fontSize: "var(--text-sm)", fontWeight: 650, display: "flex", alignItems: "center", gap: 8 }}>
            <Inbox size={16} /> Последние заявки ({ws?._count?.leads || 0})
          </h2>
        </div>

        {(!ws?.leads || ws.leads.length === 0) ? (
          <p style={{ padding: "30px 20px", color: "var(--ink-muted)", fontSize: "var(--text-sm)", textAlign: "center" }}>
            Нет заявок
          </p>
        ) : (
          <div style={{ maxHeight: 500, overflowY: "auto" }}>
            {ws.leads.map((lead) => (
              <div key={lead.id} style={{
                ...
                display: "flex", gap: 12, alignItems: "flex-start", fontSize: "var(--text-xs)",
              }}>
                <div style={{ minWidth: 40, textAlign: "right", color: "var(--ink-muted)", fontWeight: 600 }}>
                  {lead.budgetMin ? `${lead.budgetMin}₽` : "—"}
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontWeight: 500, color: "var(--ink-body)", marginBottom: 2 }}>
                    {lead.title || "Без названия"}
                  </p>
                  <p style={{ color: "var(--ink-muted)", lineHeight: 1.4 }}>
                    {(lead.description || "").slice(0, 120)}
                  </p>
                </div>
                <div style={{ fontSize: "0.6rem", color: "var(--ink-muted)", whiteSpace: "nowrap" }}>
...
function Row({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "6px 0", borderBottom: "1px solid var(--border-light)", gap: 12 }}>
      <span style={{ fontSize: "var(--text-xs)", color: "var(--ink-muted)", minWidth: 120 }}>{label}</span>
      <span style={{
        fontSize: "var(--text-xs)", color: "var(--ink-body)", fontWeight: 500, textAlign: "right",
